...
var (
	...
	// CommonQuoteMark represnets the common quote mark
...
func (q *Quoter) MustQuote(s string) string {
	r, err := q.Quote(s)
	if err != nil {
		panic(err)
	}
...
func (q *Quoter) MustJoin(a []string, sep string) string {
	value, err := q.Join(a, sep)
	if err != nil {
		panic(err)
	}
	return value
...
func (q *Quoter) MustJoinWrite(b *strings.Builder, a []string, sep string) {
	if err := q.JoinWrite(b, a, sep); err != nil {
		panic(err)
	}
}
...
func (q *Quoter) MustQuoteTo(buf *strings.Builder, value string) {
	if err := q.QuoteTo(buf, value); err != nil {
		panic(err)
	}
}
...
func ReplaceComment(remarks string) string {
